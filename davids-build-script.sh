...
cp packages/nessus_6.10.5_amd64.deb kali-config/common/packages.chroot/
cp packages/nomachine_5.2.21_1_amd64.deb kali-config/common/packages.chroot/
...
if [ -f packages/VMware-Player-12.5.5-5234757.x86_64.bundle ]; then
	printf "Adding VMware player to /root (manual install required)\n"
	cp packages/VMware-Player-12.5.5-5234757.x86_64.bundle kali-config/common/includes.chroot/root/
else
	printf "WARNING no VMware Player binary in packages folder\n"
fi

if [ -f packages/burpsuite_pro.jar ]; then
	printf "Adding Burp to /root (manual install required)\n"
	cp packages/burpsuite_pro.jar kali-config/common/includes.chroot/root/
else
...
wget https://raw.githubusercontent.com/kafkaesqu3/custom_kali/master/kali-config/common/includes.installer/preseed.cfg -O ./kali-config/common/includes.installer/preseed.cfg 
